...
(defun nalist-helper-compose-kill-buffer-forms (sym-list)
  "Compose forms from SYM-LIST for `with-temp-buffers'."
  (mapcar #'(lambda (sym)
              `(kill-buffer ,sym))
          sym-list))
...
(defmacro with-temp-buffers (sym-list &rest body)
  ...
  `(unwind-protect
       (progn
         ,@(nalist-helper-compose-generate-buffer-forms sym-list)
         ,@body)
     ,@(nalist-helper-compose-kill-buffer-forms sym-list)))
